...
parameter target_emission_reduction / 0.80 /;
...
* Constraint for renewable energy limit
eq_renewable_limit(h).. sum(technology$(ord(technology) <= 4), var_g(h, technology)) =G= 0.7 * sum(technology, var_g(h, technology)); 
...
execute_unload 'HarmonyHorizon.gdx' var_g, var_installed_cap, var_system_cost, var_emissions;;

* Export results to SQLite database
execute 'gdx2sqlite -i HarmonyHorizon.gdx -o resultsHarmonyHorizon.db -fast'; 
