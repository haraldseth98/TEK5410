...
* from: ENTSO-E Transparency PLatform, Germany 2022
parameter demand(h) /  
$include Germany_demand_Base.tsv
/;

* Hourly generated capacity factors for different technologies
* from: PV+Wind from Renewables Ninja (Germany, 2019), The rest aggregated based on literature. 
parameter capacity_factors(h, technology) / 
$include Germany_CF_Base.tsv
/;
...
parameter target_emission_reduction / 0.4 /;
...
* Constraint for renewable energy limit
eq_renewable_limit(h).. sum(technology$(ord(technology) <= 4), var_g(h, technology)) =G= 0.4 * sum(technology, var_g(h, technology));
...
execute_unload 'BaseScenario.gdx' var_g, var_installed_cap, var_system_cost;

* Export results to SQLite database
execute 'gdx2sqlite -i BaseScenario.gdx -o resultsBaseScenario.db -fast'; 
